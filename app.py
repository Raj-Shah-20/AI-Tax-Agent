...
def generate_form():
    """Generate and download tax form"""
    try:
        ...
        # Generate tax form
        form_path = generate_tax_form(data)
        
        logger.info(f"Tax form generated: {form_path}")
        
        return send_file(form_path, as_attachment=True, 
                        download_name=f"tax_form_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf")
        
    except Exception as e:
        logger.error(f"Error generating tax form: {e}")
        flash('Error generating tax form. Please try again.', 'error')
        return redirect(url_for('index'))
